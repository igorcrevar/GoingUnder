...
public class CollisionResolver {
	private float deltaTimeCollisionOccured;
	...
	public boolean detect(Player player, ArrayList<ObstacleObject> obstacles) {
		...
		playerBoundingBox.update(player.getX(), player.getY(), player.getAngle());
		playerBoundingSphere.update(player.getX(), player.getY(), player.getAngle());
		playerBoundingSphere2.update(player.getX(), player.getY(), player.getAngle());
	...
	public float getDeltaTime() {
		return deltaTimeCollisionOccured;
	}
}
